...
func TestMaliciousDetector_NoScoreInitially(t *testing.T) {
	...
	if score := d.Score("client1"); score != 0 { t.Fatalf("initial score should be 0, got %f", score) }
	if d.IsMalicious("client1") { t.Fatal("should not be malicious initially") }
}
...
func TestMaliciousDetector_HighRateDetection(t *testing.T) {
	...
	d := NewMaliciousDetector(cfg)
	for i := 0; i < 20; i++ { d.RecordMessage("client1") }
	d.Evaluate("client1")
	if !d.IsMalicious("client1") { t.Fatalf("should be malicious after high rate; score=%f", d.Score("client1")) }
}
...
func TestMaliciousDetector_TopicScanDetection(t *testing.T) {
	...
	d := NewMaliciousDetector(cfg)
	for i := 0; i < 10; i++ { d.RecordSubscribe("client1", "topic/"+string(rune('a'+i))) }
	d.Evaluate("client1")
	if !d.IsMalicious("client1") { t.Fatalf("should be malicious after topic scan; score=%f", d.Score("client1")) }
}
...
func TestMaliciousDetector_ReconnectDetection(t *testing.T) {
	...
	d := NewMaliciousDetector(cfg)
	for i := 0; i < 5; i++ { d.RecordReconnect("client1") }
	d.Evaluate("client1")
	if !d.IsMalicious("client1") { t.Fatalf("should be malicious after reconnect loop; score=%f", d.Score("client1")) }
}
...
func TestMaliciousDetector_ScoreDecay(t *testing.T) {
	...
	if scoreAfter >= scoreBefore { t.Fatalf("score should decay: before=%f after=%f", scoreBefore, scoreAfter) }
}
...
func TestMaliciousDetector_DisabledNeverMalicious(t *testing.T) {
	...
	d := NewMaliciousDetector(cfg)
	for i := 0; i < 100; i++ { d.RecordMessage("client1") }
	d.Evaluate("client1")
	if d.IsMalicious("client1") { t.Fatal("should never be malicious when disabled") }
}
