...
export interface ClusterDeviceListResponse {
  devices: DeviceSummary[]
  total: number
  self: string
}
...
export async function fetchClusterDevices(): Promise<ClusterDeviceListResponse> {
  const { data } = await client.get<ClusterDeviceListResponse>('/cluster/devices')
  return data
}
