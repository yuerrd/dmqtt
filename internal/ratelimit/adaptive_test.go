...
func TestAdaptiveController_DefaultScaleFactor(t *testing.T) {
	...
	if sf := ac.ScaleFactor(); sf != 1.0 {
		t.Fatalf("default should be 1.0, got %f", sf)
	}
}
...
func TestAdaptiveController_HighLoadReduces(t *testing.T) {
	...
	ac := NewAdaptiveController(cfg)
	ac.UpdateLoad(0.95)
	if sf := ac.ScaleFactor(); sf != 0.5 {
		t.Fatalf("high load should set 0.5, got %f", sf)
	}
}
...
func TestAdaptiveController_MediumLoadReduces(t *testing.T) {
	...
	ac := NewAdaptiveController(cfg)
	ac.UpdateLoad(0.75)
	if sf := ac.ScaleFactor(); sf != 0.8 {
		t.Fatalf("medium load should set 0.8, got %f", sf)
	}
}
...
func TestAdaptiveController_LowLoadIncreases(t *testing.T) {
	...
	ac := NewAdaptiveController(cfg)
	ac.UpdateLoad(0.2)
	if sf := ac.ScaleFactor(); sf != 1.2 {
		t.Fatalf("low load should set 1.2, got %f", sf)
	}
}
...
func TestAdaptiveController_NormalLoad(t *testing.T) {
	...
	ac.UpdateLoad(0.5)
	if sf := ac.ScaleFactor(); sf != 1.0 {
		t.Fatalf("normal load should be 1.0, got %f", sf)
	}
}
...
func TestAdaptiveController_DisabledAlwaysOne(t *testing.T) {
	...
	ac.UpdateLoad(0.95)
	if sf := ac.ScaleFactor(); sf != 1.0 {
		t.Fatalf("disabled should be 1.0, got %f", sf)
	}
}
...
func TestCalcSystemLoad(t *testing.T) {
	...
	if load < 0 || load > 1 {
		t.Fatalf("load should be between 0 and 1, got %f", load)
	}
}
